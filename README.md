...
- **Screenshots and the demo arc run on synthetic, CMS-shaped data by default.** Real CMS ingest is a one-command opt-in (`make demo-real`) but is not what the screenshots below show.
- **Auto-extracted Doctor Memory facts are unverified.** They are generated by an LLM from query results and currently have no human-review gate. Treat them as draft CRM notes, not governed knowledge.
- **Single-user local demo.** User identity is a demo cookie, not real auth; Docker uses fixed dev-only passwords. Do not load real HCP notes or internal data.
...
- **The SQL safety gate is a solid first line** (SELECT-only, read-only DB role behind it) but is not a complete adversarial-hardened gate yet.
...
Real run of the app on 2026-05-30, on the **synthetic seed data** (these are live screenshots of the running UI via headless Chrome, not mockups — but the data is synthetic, not real CMS). Each one shows a meaningful moment.
...
Alice types: *"Top 10 oncologists in California prescribing Herceptin in 2023"*

![Q1 typed and answered, still PENDING](docs/screenshots/02-q1-typed-and-answered-pending.png)

Notice the **`cold start — no verified examples retrieved`** badge at the top — there's nothing in Question Memory yet for the agent to draw on. Claude generates a clean CTE query JOIN-ing through `reference.drug_alias`. **10 rows returned in ~30 ms**, top prescriber with 593 claims. Memory counters in the right pane: still `0 / 0 / 0` because Alice hasn't clicked Verify yet.

---

### Step 3 — Days later, a DIFFERENT rep (Bob) asks a similar question — the agent uses Alice's verified example automatically

Alice's one click on ✓ Verify sent Q1 into Question Memory and triggered the background extractor (20 atomic facts about her 10 result HCPs into Doctor Memory). Then a different user signed in — **Bob** — and asked: *"Top 10 oncologists in California prescribing Kadcyla in 2023"*
...
Right pane counters: **`1 verified / 40 HCP facts / 19 HCPs known`** — Alice's Q1 still the only verified example, and Bob's query just added 20 more HCP facts via background extraction.

In this run, Alice's Q1 took ~16 seconds (cold start, no example to copy from) and Bob's Q2 took ~7 seconds (had Alice's pattern as a template). This is a single illustrative run, not a benchmark — LLM latency varies — but it shows the mechanism: a verified example lowers the next user's cold-start cost.

---

### Step 4 — Bob clicks on Dr. Chen — Doctor Memory side panel shows what Alice already knew about her, plus what Bob just learned

![Bob sees Dr. Chen's accumulated facts from Alice's earlier query + his own](docs/screenshots/04-doctor-memory-card.png)

Right side panel now shows **NPI 9994999393 (Dr. Chen)**, who appeared in both Alice's Q1 and Bob's Q2 result sets. The panel has four facts — two extracted from Alice's Herceptin query days ago, two from Bob's Kadcyla query just now. **No rep typed any of these.** All four were auto-extracted by an LLM call from the query result rows:

| From | Category | Fact |
|---|---|---|
| Alice's Q1 (Herceptin) | `prescribing_pattern` | Submitted 562 Herceptin/trastuzumab Medicare Part D claims in 2023 |
| Alice's Q1 (Herceptin) | `clinical_focus` | Hematology/Oncology specialist practicing in San Jose, CA |
| Bob's Q2 (Kadcyla) | `prescribing_pattern` | Submitted 530 Kadcyla (ado-trastuzumab emtansine) Part D claims in CA in 2023 |
| Bob's Q2 (Kadcyla) | `clinical_focus` | Practices Hematology/Oncology in San Jose, CA |

This is the team-memory effect: Bob walks into Dr. Chen's office next week, opens her profile, and gets the full team context that Alice's earlier work helped build — regardless of which drug Bob himself was looking at. He can also click "Add note" to type a free-text rep_note that travels to whoever asks about Dr. Chen next.
...
| Claim | Evidence |
|---|---|
| Agent generates correct SQL on CMS-shaped pharma data (synthetic seed; real CMS optional via `make demo-real`) | Screenshots 2 & 3 — SQL visible, 10 rows returned in ~20–30 ms each |
| Memory grows from team usage | Right pane: `0/0/0` → `1/40/19` between screenshots 1 and 3 |
...
| Team memory lowers cold-start cost for the next user | Single illustrative run: Alice's Q1 ≈ 16s cold; Bob's Q2 ≈ 7s using Alice's template. One run, not a benchmark — the point is the mechanism, not the exact multiple |
| Knowledge about individuals compounds across users (Doctor Memory) | Screenshot 4: Bob clicks Dr. Chen; sees 2 facts Alice's query established + 2 facts Bob's query just added — all auto-extracted |
...
>
> Bob, a different rep, asks "Top 10 oncologists in California prescribing Kadcyla in 2023." The agent's `retrieve_examples` step finds Alice's verified Herceptin query — similarity 0.85 — and pulls it into the prompt. The generated SQL correctly JOINs through `reference.drug_alias` on its first attempt. Provenance badge in the UI: ✓ used verified example from alice. Bob has no idea this needed any help; from his perspective it just worked.
